...
class AdminFichePersonnageRareteSsrController extends AbstractController
{
    ...
    public function new(Request $request, ManagerRegistry $managerRegistry): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $fichePersonnageRareteSsr->setImg($nomImg); // définit le nom de l'iamge 2 à mettre en base de données
            ...
            $this->addFlash('success', 'La Fiche du Personnage a été ajoutée'); // génère un message flash
    ...
    public function edit(Request $request, FichePersonnageRareteSsrRepository $fichePersonnageRareteSsrRepository, int $id, ManagerRegistry $managerRegistry): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $nomOldImg = $fichePersonnageRareteSsr->getImg(); // récupère le nom de l'ancienne img
            if ($infoImg !== null) { // vérifie si il y a une img1 dans le formulaire
                ...
                if (file_exists($cheminOldImg)) {
                    unlink($cheminOldImg); // supprime l'ancienne img
                }
                $nomOldImg = time() . '-1.' . $infoImg->guessExtension(); // reconstitue le nom de la nouvelle img
                $fichePersonnageRareteSsr->setImg($nomOldImg); // définit le nom de l'img1 de l'objet Maison
                $infoImg->move($this->getParameter('fichePersonnageRareteSsr_pictures_directory'), $nomOldImg); // upload
            } else {
                $fichePersonnageRareteSsr->setImg($nomOldImg); // re-définit le nom de l'img à mettre en bdd
            }
            $manager = $managerRegistry->getManager();
            $manager->persist($fichePersonnageRareteSsr);
            $manager->flush();
            $this->addFlash('success', 'La Fiche du Personnage a bien été modifiée');
    ...
    public function delete(FichePersonnageRareteSsrRepository $fichePersonnageRareteSsrRepository, int $id, ManagerRegistry $managerRegistry): Response
    {
        ...
        $manager->remove($logo);
        $manager->flush();
        $this->addFlash('success', 'La Fiche du Personnage a bien été supprimée');
        return $this->redirectToRoute('admin_fiche_personnage_rarete_ssr_index');
    }
}
